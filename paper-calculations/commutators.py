...
def OrbitalCount(terms):
  n_orbitals = 0
  for term in terms:
    print "orbitals count print term:", term
...
def ElectronCount(molecule):
  if molecule == 'H':
    n_electrons = 1
  elif molecule in ['H2', 'He', 'HeH+', 'H3+'] or molecule[:3] == 'H2_':
    n_electrons = 2
  elif molecule in ['Li', 'H3']:
    n_electrons = 3
  elif molecule in ['Be', 'H4', 'P4', 'D4', 'LiH', 'H2O']:
    n_electrons = 4
  elif molecule == 'B':
    n_electrons = 5
  elif molecule in ['C', 'Li2', 'BeH2']:
    n_electrons = 6
  elif molecule in ['N', 'H6']:
    n_electrons = 7
  elif molecule in ['O', 'CH2']:
    n_electrons = 8
  elif molecule in ['F','OH']:
    n_electrons = 9
  elif molecule in ['HF',  'NH3', 'CH4', 'Ne'] or molecule[:3] == 'HF_':
    n_electrons = 10
  elif molecule == 'Na':
    n_electrons = 11
...
def GetHamiltonianTerms(molecule, integral_type,
                        add_conjugates=True, verbose=False, path=None, n_orbitals=None):
  ...
  """
  print "Inside commutators: path", path
  ...
  with open(name, 'r') as file:
    for line in file:
      ...
      if len(data) == 3:
        ...
        if n_orbitals==None:
          current_orbitals = max(current_orbitals, max(term))
  ...
  # print "current_orbitals inside commutators:", current_orbitals
  # Loop over all possible terms and add the normal form.
  if n_orbitals != None:
    current_orbitals = n_orbitals
  unique_terms = {}
  for p in range(1, current_orbitals + 1):
    for q in range(1, current_orbitals + 1):
      ...
      for r in range(1, current_orbitals + 1):
        for s in range(1, current_orbitals + 1):
  ...
  return FCIenergy, repulsionEnergy,coefficients, terms
