...
do
    ...
    sed 's/DIST/'$base5'/g' mol2.template > temporal1 
    ...
    sed 's/PREPMETHOD/'$prepmethod'/g' temporal1 > inputs/$molecule-$base5-$base-$method-$prepmethod-$ntrotter-$guess$threshold-CAS-$frozen-$virt-2.sh    
    sbatch inputs/$molecule-$base5-$base-$method-$prepmethod-$ntrotter-$guess$threshold-CAS-$frozen-$virt-2.sh    
