...
do
    ...
    sed 's/DIST/'$base5'/g' mol1.template > temporal1 
    ...
    sed 's/PREPMETHOD/'$prepmethod'/g' temporal1 > inputs/$molecule-$base5-$base-$method-$prepmethod-$ntrotter-$guess$threshold-CAS-$frozen-$virt-1.sh    
    sbatch inputs/$molecule-$base5-$base-$method-$prepmethod-$ntrotter-$guess$threshold-CAS-$frozen-$virt-1.sh    
