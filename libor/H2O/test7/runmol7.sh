...
do
    ...
    sed 's/DIST/'$base5'/g' mol7.template > temporal1 
    ...
    sed 's/PREPMETHOD/'$prepmethod'/g' temporal1 > inputs/$molecule-$base5-$base-$method-$prepmethod-$ntrotter-$guess$threshold-CAS-$frozen-$virt-7.sh    
    sbatch inputs/$molecule-$base5-$base-$method-$prepmethod-$ntrotter-$guess$threshold-CAS-$frozen-$virt-7.sh    
